...
parameter:
******************************************************************************/
void GUIPAINT::Paint_SelectImage(UWORD Image_Name)
{
    Paint_Image.Image_Name = Image_Name;//At least one picture
...
void GUIPAINT::Paint_SetPixel(UWORD Xpoint, UWORD Ypoint, UWORD Color)
{
    ...
    case IMAGE_ROTATE_90:
        X = Ypoint;
        Y = Paint_Image.Image_Width - Xpoint - 1;
        Paint_DrawPixel(X, Y, Color);
        break;
    case IMAGE_ROTATE_180:
        X = Paint_Image.Image_Width - Xpoint - 1;
        Y = Paint_Image.Image_Height - Ypoint - 1;
        Paint_DrawPixel(X, Y, Color);
        break;
    case IMAGE_ROTATE_270:
        X = Paint_Image.Image_Width - Ypoint - 1;
        Y = Xpoint;
        Paint_DrawPixel(X, Y, Color);
        break;
    }
                              ...
                              UWORD Color_Background, UWORD Color_Foreground)
{
    ...
    UWORD Dx = Font->Width;
    Paint_ClearWindows(Xstart, Ystart, Xstart + Font->Width * 7, Ystart + Font->Height, WHITE);
